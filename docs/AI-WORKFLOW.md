...
`run` records a pre-Worker snapshot under `.agent/worker-snapshots/`. `review-output` compares the current worktree against that snapshot, so the Analyst sees all Worker-produced changes, including out-of-scope files, but not edits that were already present before the Worker started. The snapshot is captured **once per slug — on the first `run`** (or when no snapshot yet exists) — and **reused on retries** (a re-`run` of a prompt already in `tasks/run/`): this keeps the baseline anchored to the true pre-Worker state, so after a `changes-requested` + fix the diff reflects the Worker's *cumulative* changes, not just the last increment. (Re-capturing on every retry previously produced a truncated diff that read as a false negative.) If no snapshot is available (legacy/manual runs), `review-output` falls back to the historical whole-repo diff (`git diff HEAD` + untracked files). To avoid ambiguous concurrent output, both `run` and `review-output` still refuse to proceed when `tasks/run/` holds more than one prompt at a time:
